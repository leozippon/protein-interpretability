...
**What has run, and what the per-arm result awaits.** The endpoint qualification, the support declaration, the profiles and the control ladder are complete and frozen. The extraction is in flight: **8 of 33 arms complete with 0 failures** at the time of writing, across four lane groups on eleven cards. What the likelihood cell awaits is wall clock alone — the cohort, plan, controls and profiles are frozen and pushed, the manifests cover the roster exactly, and the fit and panel entry points are the same ones that produced the [remote-homology gate's](D1_GATE_REMOTE_HOMOLOGY.md) completed 33-arm panel, so the cell is reportable the moment the last cell exits. Nothing about which arms finish first is a function of any outcome.
