...
**Two gates cannot serve a depth selection from anything they retained.** The folding-and-stability gate and the residue-interaction gate each hooked the block after half the stack and the last block on their own cohorts — 101 MegaScale natural family groups and 64 held groups of double-mutant cycles respectively — and no other block's state was ever written for either. A depth-resolved recomputation of their representation cells needs a fresh forward pass over those cohorts: 33 arms at batch size one on cohorts of 25,856 and 8,192 measured rows, which is a measurement campaign of the same order as the original extraction. This is better known now than when the recomputation runs, and it bounds what a depth-resolved reassessment can say about those two gates without new compute. Their class-level recomputation is unaffected.
...
What removes it is not a full depth sweep of those cohorts but a targeted set: each arm's **selected** depth plus the two admitted depths retained alongside, so the new cells are comparable with the published ones on the same arrays. An arm whose selected depth already coincides with an admitted one hooks two rather than three. `src/transfer/targeted_depth.py` holds the plan at content digest `2e16d92eddc96c1bc5f63687e977a8886f3c9d31133f2af1076e12e39c2eb03d` and `scripts/transfer/build_targeted_depth_campaign.py` writes it.
...
| Variant | Depths hooked over the panel | Stability cohort | Cycle cohort | Total | Resolves any later depth |
| --- | ---: | ---: | ---: | ---: | --- |
...
**One gate stands before the wave, and it is the property no test in this repository can otherwise reach.** `extract_batch`'s hooking loop needs a loaded checkpoint, so the order in which it registers hooks and concatenates their captures is preserved by construction rather than by measurement — and if that order moved, every archive in the sweep would be silently wrong. The gate is therefore one arm extracted with `--every-block`, whose feature positions 0 to 3 must come out byte-identical to that arm's already-retained archive. It is a real end-to-end check on a loaded checkpoint against a known-good artefact. A mismatch is a defect in the shared hooking rule, not a tuning problem, and stops the dispatch.

**It has not run, because no card in the allocation is idle.** Measured twice about twenty minutes apart across all four pods, every one of the eleven cards holds between 5,881 MiB and 128,255 MiB, against the campaign queue's 1,000 MiB idle rule, which refuses a cell on a busy card rather than launching it. Several cards report 0% utilisation while holding 25 to 28 GiB, which is another campaign's loaded checkpoint rather than free capacity. The idle threshold was not raised and no card was co-tenanted; both are disciplines the gate records state as claims, and forcing either to make this gate run would trade a real constraint for an apparent one. The checkpoints exist only on the allocation, so the gate cannot be moved to the workstation either: it holds the stability cohort and its plan but no arm's weights.

So the sweep is queued behind capacity rather than behind a decision. The two commands are in the evidence section, and the gate's verdict governs whether the 32-arm wave follows.
...
5. **An idle card for the one-arm hooking gate**, and then the 32-arm wave behind it. The variant is decided — every block, 18.487 measured GPU-hours, 720.18 GiB — and the manifests are written; what is missing is capacity, not a choice.
