...
The same measurement the folding-stability gate reports, on this gate's own cohort, folds, control ladder, ridge grid and contrast form, with the same four qualifications: one axis, unadjusted for multiplicity, **512 coordinates** — one block's `mean` and `last` at 256 each under the declared depth projection — against the published **1,024** pooled across two depths, so **not comparable to the published increment**. Seven arms, 45 blocks, three split seeds, 135 fits, no failures, at this gate's four pinned BLAS threads. The control fired on all **135 of 135**: with only the representation block varying, the baseline predictions are identical across an arm's depths at a given seed.
