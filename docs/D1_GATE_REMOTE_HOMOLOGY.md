...
One thing has to be said before any number. This endpoint is **not** external confirmation of anything. It shares the cDNA-display proteolysis assay technology and an author with Tsuboyama 2023, which is one of the two development sources, so it reproduces the shared-source defect the catalogue records as L50 and may not be read as independent replication. What it supplies is **remoteness, not independence**.

**How this gate stands to the external-confirmation gate, and why neither is a weaker version of the other.** The two gates added this cycle are complementary **by construction**, and the division is fixed by measurement on both sides rather than by preference. This gate supplies remoteness without independence: **100 entirely remote family groups against the existing cohorts' 1, 2 and 12**, on an assay it shares with a development source. The [external-confirmation gate](D1_EXTERNAL_CONFIRMATION.md) supplies a third source, a third assay and a different measured quantity **without remoteness**: banding all 428 of its retained wild types against the same UniRef50 search its profiles came from gives **426 near duplicates, 1 at 70–95% identity and 1 with no reported hit, so 95 of its 96 family groups are entirely close and none is remote**. That gate therefore has no remote stratum to read at all — not a thin one — and the three-outcome frame below does not apply to it; it confirms where a corpus retrieves a near relative of essentially every target, and this gate populates the band nothing else reaches and confirms nothing. Reading either as a substitute for the other misreads both.
...
**What a band is and is not, and how it differs from the grouping.** Two measurements are used here against two different references and they are orthogonal; conflating them would turn a statement about independence into a statement about biological distance, or the reverse.

* **The grouping governs independence among cohort members.** It is exact Smith-Waterman between the retained wild types themselves, an edge at 30% identity and 80% coverage of both. It says which units may be held out from each other and nothing about any corpus.
* **The band governs remoteness, and it is measured against an external reference corpus.** Each wild type is searched against the full UniRef50 index — `uniref50_full.dmnd`, 60,315,044 clusters — under `diamond blastp --very-sensitive --masking 0 --evalue 1e-3`, and its band is the identity of the best hit as `100 * nident / qlen`. A remote group is one every member of which falls below 70% identity **to that external index**, not one that merely differs from the other cohort members.

So the remote stratum is remote from a reference corpus and not by within-cohort banding. The residual boundary is a different one, it is stated rather than hidden, and it can be made specific from the panel's own declarations: **the searched snapshot is not the corpus most of these arms were trained on.** `src.transfer.arms` records each panel member's `pretraining_corpus` from its own model card with an `undeclared` sentinel rather than a guess, and of the 33 roster arms exactly **one — ProtGPT2 — declares the corpus family that was searched**. Two, ProGen2-medium and ProGen2-small, declare **UniRef90 + BFD30**, which is larger and differently filtered, so a background remote from UniRef50 is not thereby remote from what they saw. **Nineteen arms carry no corpus declaration in the repository at all**, including the ProGen3, ProLLaMA, ProteinGLM, RITA, InstructProtein and Galactica families. And for the text arms the band is uninformative in the other direction: a protein sequence's identity to a UniRef entry says nothing about whether a web corpus contained it.

So a band is remote **from one snapshot of one reference collection**, no band here is per-arm, no detected alignment is absence from any model's pretraining corpus, and a remote group is a conservative held-group control rather than a certified remote-family holdout. Tightening this into a per-corpus claim is a measurement — the same corpus-distance operation run against each identifiable collection — and it is scoped in [D1_REMOTE_ACQUISITION_HANDOFF.md](D1_REMOTE_ACQUISITION_HANDOFF.md) rather than assumed here.
...
**This table is provisional and carries no verdict.** It is conditional on one compressed linear readout of four pooled block summaries at **the two depths this extraction hooked and no others** — the middle block and the final block, which is all the entry point computes. The readout-class reassessment has published its selection, class `C4_full_random_feature` with per-(arm, panel) depths, but the gate-level recomputation has not run, so nothing below is a statement about what these representations contain and in particular nothing below is a not-detected verdict. L53 records the hazard exactly: a gate fitted at the admitted depth produces a well-formed increment with no indication another depth exists. The full-width states are retained, 7.67 GiB over 10,065 archives, so a readout-class refit at those two depths is a refit rather than a re-extraction; the every-block pass for this cohort is dispatching.
...
## Why this endpoint is measurement-limited: the full statement

Two measurements together say why, and neither is sufficient alone. The first is about the assay, the second about the stratum on which a control should have been easiest, and the conjunction is what closes the endpoint rather than deferring it.

**The channel disagreement is systematic, not imprecision a filter can remove.** A full-file sweep of the interval rule over a tenfold range — every one of the 2,287,291 rows carries both channels and a combined 95% interval — leaves the effect-scale shared-to-discordance ratio flat and then falling: 1.702 at a width rule of 1.0 kcal/mol, 1.688 at 0.5, 1.672 at 0.4, 1.678 at 0.3, 1.677 at 0.25, 1.646 at 0.2, 1.673 at 0.15 and **1.447 at 0.1**, while the backgrounds carrying at least eight admitted single substitutions collapse from 5,987 to 99, a 98.3% loss. The mechanism is visible in the components: tightening the rule removes high-variance states and the shared component and the discordance shrink in almost exact proportion, 0.6784 to 0.4169 against 0.3986 to 0.2880 kcal/mol, so the ratio does not move; the effect standard deviation falls from 0.7200 to 0.4503 as fast as the noise does; and the offset survives at every threshold, +0.065 to +0.133 kcal/mol on the effect scale. **So the two proteases report systematically different quantities on these sequences, and no setting of the reported-precision filter approaches the admitted MegaScale endpoint's 3.86 [3.61, 4.13].** "The floor is too high" is a statement about the assay, not about the models, and it is not improvable by quality control.

**On the close-identity stratum, where a control should be easiest, no protein-pretrained arm holds a positive sign across three resamplings.** Of the 33 arms, only **3** carry a positive close-stratum increment at all three split seeds — dialogpt-small, gpt2 and protgpt3-1.3b — and **none of the 11 residue-level arms is among them**. Nine of those eleven are positive at exactly one seed of three, and for nine the third seed is negative by *more magnitude* than the first is positive: ProGen2-medium reads +0.00024, −0.00058, −0.00237 kcal²/mol², RITA-XL +0.00014, −0.00080, −0.00189, ProLLaMA Stage 1 is negative at all three. **That is not a quantity near zero awaiting precision. It is a sign that does not survive resampling one support, and narrowing an interval cannot change a sign.**

Together they are the full statement: the disagreement between the two proteases is systematic and unfilterable, **and** on the stratum where retrieval makes the question easiest no protein-pretrained arm holds a stable sign. That is why every arm falls in the third declared outcome, and why the result is **measurement-limited rather than model-negative**.

### The expansion was refused on the control, not on cost

One reading of a closed endpoint is that the budget ran out, and that reading is wrong here, so the arithmetic is recorded. A group-count calculation from the published intervals — standard error recovered as half-width over 1.959964, required count `n* = n0 · (1.959964 · se / delta)²`, largest of the three seeds — says an expansion **would** have worked on the remote stratum: ProGen2-large and ProGen2-xlarge needed only **55 and 57 groups against the 100 observed**, which is why they resolved there, and qwen3-8b-base and RITA-XL would have reached at **179 and 194**. A few hundred remote groups were attainable at roughly 70 minutes of UniRef50 search plus a re-extraction.

It was declined anyway. On the close stratum the same calculation gives **173 groups for the first arm to fire, and that arm is GPT-2**, with ProtGPT3-1.3B at 4,527 and DialoGPT-small at 13,281; for the other 30 arms, the residue arms included, no group count applies at all because the sign condition fails before the width condition. A positive control whose first firing arm is a text checkpoint is not a positive control for protein-sequence information. So the expansion's purchase would have been **more remote resolutions under a control that still could not fire on a protein arm** — the configuration the three-outcome rule exists to refuse, and the one that already flagged two arms here as licensing nothing. Buying it would have manufactured the appearance of a remote result out of a control that had failed.

**What the calculation cannot reach, recorded as the limit rather than assumed.** Whether the close-stratum sign stabilises at two or three hundred groups is not estimable from 66. `n*` scales as the inverse square of the increment and is correspondingly sensitive — GPT-2 needs 690, 173, 43 or 19 groups at 0.5, 1, 2 or 3 times the observed value — but no multiple touches the residue arms, whose requirement is undefined while the sign fails. The expansion's case therefore rests on an unmeasured hope rather than a calculable gain, which is what makes this a closed question rather than a deferred one. The audit is `results/remote_support_audit_20260925/`.
...
## The equal-capacity depth contrast, and a cohort disagreement

The same measurement the folding-stability gate reports, on this gate's own cohort, folds, control ladder, ridge grid and contrast form, with the same four qualifications: one axis, unadjusted for multiplicity, **512 coordinates** — one block's `mean` and `last` at 256 each under the declared depth projection — against the published **1,024** pooled across two depths, so **not comparable to the published increment**. Seven arms, 45 blocks, three split seeds, 135 fits, no failures, at this gate's four pinned BLAS threads. The control held on every comparison that could have failed: with only the representation block varying, the baseline predictions are identical across an arm's depths at a given seed. **135 comparisons are recorded and 114 can fire**; the other **21** are each arm's first measured depth compared with itself, one per arm per split seed, which no corruption of any depth could turn into a failure, so the firing count is the one to read (L59).

| Arm | Admitted depths | Selected | Selected mean | Best admitted | Difference | Seeds resolved | Highest depth measured |
| --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| ProGen2-large | 15, 31 | 25 | +0.009188 | +0.009738 | −0.000550 | 2/3 | 15 at +0.009738 |
| ProGen2-medium | 13, 26 | 23 | +0.008423 | +0.006963 | **+0.001460** | 3/3 | 6 at +0.014756 |
| ProGen2-xlarge | 15, 31 | 28 | +0.001143 | +0.003232 | −0.002089 | 0/3 | 0 at +0.007772 |
| ProGen3-112M | 4, 9 | 9 | +0.003613 | +0.008091 | — (selected is admitted) | 1/3 | 2 at +0.015558 |
| ProGen3-3B | 11, 23 | 11 | +0.003590 | +0.005251 | — (selected is admitted) | 0/3 | 6 at +0.015370 |
| ProteinGLM-7B-CLM | 17, 35 | 30 | +0.003305 | +0.004910 | −0.001606 | 0/3 | 9 at +0.014924 |
| ProtGPT2 | 17, 35 | 21 | +0.002777 | +0.012877 | **−0.010100** | 0/3 | 17 at +0.012877 |

**Every count in this section names its population, because two populations exist here and only one of them supports a difference.** Seven arms are measured; **five** produce a comparison, since on ProGen3-112M and ProGen3-3B the reassessment selected a depth the admitted extraction already hooked, so there is no selected-minus-admitted difference to take. A count over arms measured and a count over arms that produce a comparison are different quantities — the same shape as a zero quoted without its denominator, and as a count over declared metrics read as a count over data — so each is written with its population attached rather than left to inference.

**On this cohort the readout panel's depth selection mostly does not transfer**, and that is the opposite of what the same measurement returns on the folding-stability cohort. Of the five arms whose selected depth is distinct from both admitted ones, it carries **less** here on four and more on one; on folding stability it carries more on four and less on one. Only **ProGen2-medium** exceeds on both, and only **ProGen2-large** falls short on both. The two cohorts were measured with the same seven arms, the same block rule, the same 512 coordinates and the same declared projection, so what differs between them is the endpoint and its support.

**The defensible statement is therefore about the selection's scope, not its quality.** A depth chosen because it resolves on the readout panel is not thereby the depth that carries the most transferable information on an arbitrary cohort: whether it beats the admitted pair depends on the cohort, and on this one it usually does not. Two further facts sit with it. The selected depth's increment resolves above zero at all three seeds on **one of the five arms in the comparison population** here, against **four of five** on folding stability — and over all **seven measured arms**, which is the other population and not the one a difference statistic lives in, the pair is **1 and 6**. So most of these differences are between quantities that do not separate from zero at all. And the highest-reading depth is an unselected, unadmitted block on five of seven arms — often an early one, depths 0, 2, 6 and 9 — which says the same thing the other cohort said: the selection finds a better-than-admitted depth at best, never the best one.

## Limitations

**This endpoint is not independent of the development stability axis.** It shares the cDNA-display proteolysis assay technology and an author with Tsuboyama 2023, so agreement with the folding-and-stability gate would be agreement within one assay principle and would not be replication. It supplies remoteness, not independence, and confirmation is the [external-confirmation gate's](D1_EXTERNAL_CONFIRMATION.md) work on a third source and a third assay — a gate which, by the banding measured on its own 428 wild types, carries no remote stratum at all and so cannot do this gate's work either.

**The floor is the binding limitation and it is not a caveat on the numbers.** The reproducible component exceeds the per-channel discordance by 1.566 [1.301, 1.847] against 3.86 [3.61, 4.13] on the admitted MegaScale endpoint, and on the remote stratum alone the ratio's lower bound touches 1.04. A remote increment inside that floor is unresolved and is not a null, and it is reported as one of the three declared outcomes or not at all. The floor is also **not improvable by quality control and not the whole reason**: the section above gives the full statement, which pairs the systematic channel disagreement with the close stratum's sign instability.
