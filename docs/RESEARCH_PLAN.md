...
| `17_train_transcoder.py` | 2 | **outside the panel contract by design**, same reason, and `--arm` likewise reaches a dense panel arm — each streaming the corpus its own evaluation cohort is drawn from, rendered through the panel declaration. Trains a CLT and a PLT under one protocol — same data, order, seed, step budget and optimiser — so ProGenMech's CLT-beats-PLT claim is gateable despite their CLT weights being unobtainable. A third width is the parameter-matched control (L25) |
| `18_das_subspace.py` | 4 | **built, and D3.f closed before it was run.** Distributed alignment search on dense arms. Retained because the reasoning and the gates are the record; not scheduled, and not to be scheduled without the negative controls §9's D3.f row names |
| `19_routing_locality.py` | 2, 4 | **outside the panel contract by design**, same reason. Whether a MoE replacement's residual is structured by the routing decision, against a random grouping at matched cardinality and against residue identity, on sequence-disjoint folds |
| `20_retrieval_bound.py` | 1, 3 | **outside the panel contract by design**, same reason: its arms are ProGen3-112M, ProtGPT2 and ProGen2-medium. The second subtraction the fitness evidence is missing — MODEL − LOOKUP, where LOOKUP is a site-independent position-specific profile of the arm's own pretraining corpus, so that knowledge the model *acquired* is separable from knowledge it *retrieved*. Measured against wet-lab phenotype over all 217 ProteinGym substitution assays, bootstrapped over wild-type families at 50% identity, stratified by corpus identity and homologue depth. Its ``score`` stage is the only one that needs a GPU |
...
- Non-canonical reader summary, do not cite as a claim source: repository-root `SUMMARY.md`
