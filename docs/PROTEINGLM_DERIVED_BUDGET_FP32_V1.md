...
After the 7B model is loaded and numeric local tensors have been released: synchronize, `reset_peak_memory_stats`, then **one** public `scored_tokens` call on `[longest, longest, longest]` with `batch_size=1` and `max_len=1024` (three internal consecutive batches of the 1021-residue case). Duration and allocated/reserved peaks are collected after that public call returns or raises, still before the extra independent reference. Baseline, device total, phase, attempted versus completed public-call counts, input identity, and the expected 3 sequences / 3060 targets live in the same progress mapping the caller later stores on a failed artefact.

`resource_public_call_count=1` and `finite=true` only after the resource gate succeeds. An OOM must not write those success marks. A single telemetry reading that fails is recorded as `unknown` plus a sanitised error; it is never invented as 0, and it must not replace the primary exception. Missing required CUDA telemetry cannot mint `derived-budget-interface-qualified`. No extra VRAM percentage cap. No three independent public calls, no short-string substitute, no window shrink, no OOM fallback. Batch-1 qualification does not generalise other batch sizes. A failed run keeps the complete numeric case records already obtained, plus any `strict_load` / `serving_provenance` taken from this Arm, without re-deriving them after the fact.
