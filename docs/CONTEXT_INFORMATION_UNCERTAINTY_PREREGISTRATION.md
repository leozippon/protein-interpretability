...
**Where the campaign departed from this plan, recorded 2026-08-20 (EXP-R2-216).** Two departures, both narrowing and neither a change made after the numbers existed. E1 scored **thirteen** arms rather than fifteen: `bygpt5-small-en` and `bygpt5-base-en` are declared in `PANEL` but not in the campaign panel, so they remain unscored on this estimand anywhere and their absence is not a pass. And of E3's two negative controls only the within-record token shuffle ran — no stage exposes a randomly-initialised matched-architecture mode, so that control is unrun rather than negative. Everything else executed as frozen; the results and what they do to the criteria are in the audit's §5.06.
...
The two campaigns also disagree about the arms' levels by more than three times the gap being read — `progen2-base` moves 1.06 to 1.23 and `progen2-medium` 1.11 to 1.31 between them, at the same skip — and no reading of any of the four carries a valid interval. This is recorded now, as a retraction candidate against whatever text cites that ordering, because it follows from artefacts already on disk and does not depend on anything this campaign will produce.
