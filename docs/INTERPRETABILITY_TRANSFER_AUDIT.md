...
6. **The two readings of D2.c partition the panel differently (EXP-R2-099/100).**
   ...
   |---|---|---|---|
   | **size** (ΔM-gap ÷ clean M-gap) | **7.18%** | **0.89%** | **yes, no overlap** |
   | ranking (census-to-causal ρ) | +0.4685 | +0.2145 | **no** |

   By size the four protein arms sit in a tight band — ProtGPT2 0.77% and 0.49%,
   ProGen2-base 0.65%, ProGen2-medium 0.62%, ProGen2-small 0.89% — below every text
   arm-condition, and gpt2-medium **raised** the text floor rather than breaking it,
   at 14.98% (EXP-R2-105). By
...
7. **What the census contributes is directional, and only on text (EXP-R2-099).**
   ...
   **Re-checked on two ProGen2 arms and in both conditions (EXP-R2-100), and the
   ...
   protein arm shows the enrichment in either direction.** *Qualified by EXP-R2-105,
   and the qualification is recorded rather than deferred:* **gpt2-medium reads only
   −0.05** on one draw, nearer the protein band than gpt2-large, so the property may
   belong to gpt2-large rather than to text decoders. One draw does not overturn a
   six-draw claim; EXP-R2-106 takes gpt2-medium to K=3 and EXP-R2-100's gpt2-xl lanes
   are the independent check. **Until they land, this item is stated of gpt2-large
   and not of text.** ProGen2-base's −0.26 under the measurability restriction against −0.02
