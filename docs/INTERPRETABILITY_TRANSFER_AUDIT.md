...
**A second part-2 result now stands beside it, on a different mechanism and a different statistic — and unlike the one above it needs no census at all (EXP-R2-099).** D2.c was posed as *"does effect size separate arms where prevalence cannot?"*, and every statistic run against it until now answered a *ranking* question instead, because ranking is what L22 defined. Answered directly, from the instrument's own A4 record: **the strongest prediction-addressed head moves the margin by the same absolute amount on both modalities — 0.0257 nats on gpt2-large against 0.0259 on ProtGPT2 — and the separation is entirely in the denominator.** The protein arms operate at a 4–9x larger clean M-gap, so the identical intervention is a far smaller perturbation of the decision: **0.40–2.16% of the arm's own clean margin on ProtGPT2 and 0.60–0.73% on ProGen2-base, against 4.10–10.77% and 5.80–8.57% on matched gpt2-large controls.** Disjoint on every draw, 1.9x and 7.9x at the most adverse pairings. Two biases in the statistic run *toward* the null of no protein deficit — A4 is a maximum over the grid, which favours the arm with more heads (720 against ProGen2's 432) and the arm with noisier estimates (ProGen2) — and a third fact runs the other way and is recorded rather than buried: **A3 passes on every arm, and ProtGPT2 carries more confidently suppressive heads than gpt2-large** (206 against 127 of 720). A suppressive population exists on protein. *One irreducible limitation:* the normalisation is the module's own (`delta_m_gap_relative_to_clean`), but a larger clean margin also means the selected positions are simply easier to predict, so the ratio mixes mechanism strength with task confidence and cannot by itself separate the two.
...
6. **The two readings of D2.c partition the panel differently (EXP-R2-099/100).**
   ...
   **One limit on the size result, and it is material.** That statistic falls
   monotonically with parameter count across *both* families measured — gpt2-medium
   355M 13.13%, qwen 0.5B 12.29%, gpt2-large 774M 7.61%, gpt2-xl 1558M **3.19%**,
   roughly halving per doubling — while the protein arms fall far more slowly
   (ProGen2-small 151M 0.89% to ProtGPT2 774M 0.49–0.77%). It is not a head-count
   artefact: A4 is a *maximum* over the grid, so gpt2-xl's 1200 heads bias it **up**
   against gpt2-medium's 384 and it still reads lowest. So **the like-for-like
   comparison is at matched scale, where the gap is ~12×** (gpt2-large 7.61%
   against ProtGPT2 0.49% and ProGen2 0.62–0.65%), while the whole-panel 3.6×
   rests on gpt2-xl and **the trend gives no licence to extrapolate past 1.5B** —
