...
5. **Run — EXP-R2-098, and it splits the protein family.** ProGen2-base reads
   ...
   **The split is a measurement; its explanation is not identified, and this panel
   ...
   which settles nothing. ZymCTRL would be the fourth residue-level protein arm and
   would break the tie, but it is irreducibly excluded from D2.c's shared window.
   Recorded as a limit of the panel rather than resolved by argument. **EXP-R2-103 completes the family:
