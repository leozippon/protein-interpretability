...
>
> *ProtGPT2's 4.5× is EXP-R2-126's published figure and is **under re-derivation**, not settled. Read off the panel root through the versioned statistic at K=5 the same arm gives **9.0×**, its worst draw still above chance; EXP-R2-126's population mixes draws read through the versioned function with draws read through the advisory reduction that preceded it, and the two do not reproduce each other. The discrepancy runs **toward** the account this table supports, which is why it is being verified before either number is quoted as final. Neither reading changes the arm's side of the classification, and the ZymCTRL and ProGen2 figures are unaffected — every one of them was written by the versioned function into its own run's report.*
>
