...
- **Text-versus-protein feature crosscoders.** A crosscoder compares two models on
  *paired* inputs; text and protein decoders share no input, so the object is
  undefined. The construction is legitimate only where pairing exists — one model
  before and after fine-tuning, or retrieval toggled on and off.
