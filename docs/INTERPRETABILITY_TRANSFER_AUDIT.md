...
**A second part-2 result now stands beside it, on a different mechanism and a different statistic — and unlike the one above it needs no census at all (EXP-R2-099).** D2.c was posed as *"does effect size separate arms where prevalence cannot?"*, and every statistic run against it until now answered a *ranking* question instead, because ranking is what L22 defined. Answered directly, from the instrument's own A4 record, and now on **ten arms** (EXP-R2-105/107/109): **the absolute effect does not separate the modalities at all.** Excluding dialogpt-small (below), the strongest prediction-addressed head moves the margin by 0.0156–0.0626 nats on six text arms and 0.0089–0.0159 on four protein arms — **ProtGPT2's 0.0159 exceeds gpt2-xl's 0.0156**, so the two scales touch. **What separates, cleanly and without needing any interpretability instrument, is the denominator:** mean clean M-gap 0.270–0.518 on text against 0.996–3.297 on protein, disjoint at ~1.9x. **And that difference is about recurrence rather than about confidence (EXP-R2-111).** Measured at *all* scored positions on the same cohorts — no census, no knockout, no head grid — the same margin **does not separate the modalities at all**: text 1.466–1.913 against protein 1.110–1.852, overlapping, and on medians the protein arms are the *less* confident ones. What separates is the **ratio of the recurrence-position margin to the all-position margin: 0.18–0.26 on four text arms against 0.90–1.78 on four protein arms**, disjoint at 3.5x. **Text decoders are four to five times less confident where a token recurs than at a typical position; protein decoders are no less confident there, and ProtGPT2 is markedly more so.** Their ratio therefore separates too — text 3.19–18.40% against protein 0.49–0.89%, no overlap — but the result must be stated as *the same absolute intervention is a much smaller fraction of a protein decoder's decision margin*, **not** as "the mechanism is weaker in protein models", which the absolute scale refutes. **This is the most economical account currently available of why prediction-addressed machinery looks weak on protein arms, and it connects D2.c to L22:** in text a repeated token is informationally loaded and deciding whether to repeat it is a real decision — precisely the task induction and copy-suppression circuits exist to perform — while in a 20-letter alphabet over a 192-residue window a residue recurring is near-inevitable and carries little information. On this evidence the instruments are not failing to see a mechanism so much as there being much less of a *task* for one to solve. *That last sentence is interpretation of the measured ratio, not a second measurement, and is labelled so.* Two biases in the statistic run *toward* the null of no protein deficit — A4 is a maximum over the grid, which favours the arm with more heads (720 against ProGen2's 432) and the arm with noisier estimates (ProGen2) — and a third fact runs the other way and is recorded rather than buried: **A3 passes on every arm, and ProtGPT2 carries more confidently suppressive heads than gpt2-large** (206 against 127 of 720). A suppressive population exists on protein. *Two limitations, both measured rather than anticipated.* The normalisation is the module's own (`delta_m_gap_relative_to_clean`), but a larger clean margin also means the selected positions are simply easier to predict, so the ratio mixes mechanism strength with task confidence and cannot by itself separate the two. And **its denominator is a free-running quantity that can cross zero**: dialogpt-small, the off-distribution text arm, has a mean clean M-gap of **−0.785** — at most PAA-selected positions it is not predicting the antecedent token — so A4's division by `max(|mean|, 1e-9)` leaves its 5.00% without a defined sign. That arm is excluded from the size comparison and the reason is recorded; a mean is not a safe summary of this denominator.
