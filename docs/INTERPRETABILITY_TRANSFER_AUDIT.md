...
**Status:** active; canonical analysis document **Supersedes:** repository-root `check.md`, which is frozen and no longer maintained **Updated:** 2026-08-04 (through EXP-R2-122)
...
| **D2.c** | **STATUS 2026-08-04, and one result now clears every bar (EXP-R2-126).** On the re-measured panel — A1 PASS on every arm, one matched condition (ban 3 / n=600, width 192), layout **and** decoy guards, the versioned statistic, three corpus draws — **all three ProGen2 censuses retrieve the causally important heads at or below their own chance level on every one of nine draws** (ProGen2-medium 0/20 three times; 0.0–1.1× chance across the family), against text arms at 8–14 of 20 and no draw below 2.9×. The all-grid separation among K≥3 arms is complete: worst text draw **+0.6034** against best protein draw **−0.0604**, no overlap in 18 draws, and unlike the induction separation **it survives depth control** (text +0.45…+0.63 within-layer against protein −0.12…+0.01). **This is a lineage claim, not a modality one:** ProtGPT2 is at K=2 with a small-*positive* ρ and the panel's least stable retrieval (5/20 then 0/20), and ZymCTRL is structurally excluded from this window, so the protein side is one lineage plus one unstable arm. What it supports is narrow and useful — *a PAA census on a ProGen2 decoder does not recover the causally important heads at all*, which is the condition under which a published head count carries no information about the mechanism. **Everything else in this cell is under re-measurement: every EXP-R2-116 and EXP-R2-117 correction is withdrawn (EXP-R2-119) — see the banner in §1 for the mechanism.** In one line: the offline layout correction could only be applied to the causal side, because the census score is a per-sequence aggregate; the rebuilt pools give ProtGPT2 **+0.1806** and gpt2-large **+0.4425** at ban 3 / n=600, not the −0.088 the offline pass reported. That is K=1 per arm and no value in this cell may be cited until EXP-R2-121 lands K≥3 and the ProGen2 arms are re-run at the matched n=600 condition. **The text below is the record of the reasoning and its numbers are not current.** Same on copy suppression — does effect size separate arms where prevalence cannot? Directly answers the measurability question §1 leaves open. **All blockers cleared and the attainability gate passed (EXP-R2-078, EXP-R2-082, EXP-R2-087): the matched pair enters at `--width 192` with the cohort band unchanged, at zero L13 exposure.** The gate was re-specified before it was read — the +0.53 to +0.65 target is restricted-range, unmatched-score and reproducible only to ±0.12 at fixed width — and passed against gpt2-large's own induction band: **+0.4515 [+0.401, +0.498]** exhaustive at width 192, inside +0.428 to +0.535. ZymCTRL is irreducibly excluded from any shared window. **Run and decided (EXP-R2-094, three draws): ProtGPT2 +0.154 to +0.244 against the matched control's +0.449 to +0.482 — a partial transfer, not the near-zero induction reading, so the L22 failure does not reproduce on a second mechanism.** EXP-R2-096 then found the aggregate statistic reports the opposite of the stratum a census publishes, on both mechanisms; see §8 item 0. **K=6 changed nothing (EXP-R2-097): three further draws on each arm landed inside the existing ranges, endpoint for endpoint, and the stratified separation widened — ProtGPT2 +0.640 in the top 20% against +0.202, hit@20 8.5/20 against 4.7/20, sign-stable on all six draws.** **The question is now answered on effect size as well, which is how the row poses it (EXP-R2-099): the absolute effect is identical across modalities (0.0257 against 0.0259 nats) and the arms separate 1.9–7.9x once each is expressed as a fraction of its own clean margin.** **~~And D2.c turns out to have two answers inside one modality.~~ WITHDRAWN (EXP-R2-116): that result was ProtGPT2's FASTA line-break token.** `prediction_addressed.py` carries no layout-token guard, while `circuits.py` excludes line breaks from the unigram support in `fit_unigram` for exactly this arm and exactly this reason; ProtGPT2's rendering wraps at 60 residues (`arms.py:1061`), so the wraps are *interior* and the census admits them as instances. Measured on the retained pools they are **28–33% of ProtGPT2's PAA instances and carry 108–110% of its clean-margin mass** — the residue instances net negative — against 4–5% on gpt2-large and **0.0%** on all three ProGen2 arms. Excluding them uniformly on every arm, **ProtGPT2 reads −0.088 (ban 3) and −0.114 (ban 20), sign-flipping on 9 of 9 draws, and all four protein arms are negative.** The corrected picture is *simpler* than the withdrawn one and **strengthens the modality result**: text +0.276 to +0.592 against protein −0.234 to −0.088, gap **+0.364** against a within-protein spread of +0.146, **ratio 2.49** — where the published figures were gap +0.170 and ratio 0.38, on which this document had demoted the ranking statistic as a poor modality instrument. The retained figures below are left in place with their corrections marked, because the arithmetic of the withdrawal matters. **Two further structural checks were then raised by an independent review and reproduced (EXP-R2-117), and the separation survives both:** depth-controlled (within-layer, since the causal readout is depth-biased by construction — `r(layer, |ΔM-gap|)` is +0.28 to +0.72 on every arm) the gap is **+0.197**; under `paa_specific`, the score the shipped selector actually ranks on, it is **+0.212**. **One qualification is now permanent: individual protein arm signs are not robust to the score choice** — "all three ProGen2 arms are negative" holds on `paa_specific_matched` and *reverses* on `paa_specific` (+0.07 to +0.15). Both columns must be published together. The modality statement does not depend on the choice. **The old text follows.** (EXP-R2-098/099/100.) ProGen2-base reads **[−0.292, −0.159]** and ProGen2-medium **[−0.282, −0.139]** against a matched ban-3 text control at [+0.454, +0.507]. Run in ProGen2's exact condition — ban 3, n=200 — ProtGPT2 reads **[+0.187, +0.232]**, a move of **+0.012** from its own ban-20/n=600 range, so **the split is a property of the arm and not of the condition**, and the two ProGen2 arms, which are architecturally identical down to the parameter count and differ only in pretraining corpus, agree. **The two readings of D2.c behave very differently as modality instruments, which is the deeper result.** Both separate text from protein — that much is now measured rather than asserted, correcting an earlier statement here that the ranking statistic did not. What separates them is how the cross-modality gap compares with the spread *inside* the protein modality: for **effect size** the gap is 2.30 points against a within-protein spread of 0.41, a ratio of **5.6**; for **ranking** the gap is +0.170 against a within-protein spread of **+0.449**, a ratio of **0.38** — the protein arms differ from each other by nearly three times what they differ from text. A statistic with more structure inside a modality than across it is a poor instrument for a modality claim, and that is L22's statistic. See the D2.c section below | **1–2 GPU-h** for the check; panel cheaper than the 12–25 previously carried, since width 192 is ~2.7x cheaper per forward |
