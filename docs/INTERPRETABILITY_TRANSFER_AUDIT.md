...
|---|---|---|
| **D1.d — joint-model foundational bridge** | Which differences remain when text and protein modes share one checkpoint? | Qualify one dense joint decoder first: strict load, native rendering, scored positions, output semantics, and context-information checks for both modes. Same-checkpoint comparison controls shared weights and most architecture, not tokenization, prompts, output space, or training exposure. Add a joint MoE only if the dense result leaves routing or expert specialisation as a live explanation. **No qualified bridge exists yet (EXP-R2-151, 2026-08-10; figures as corrected the same day by `21_joint_mode_qualification.py`).** The two reachable dense joint decoders fail on opposite modes at identical architecture and scale — both `OPTForCausalLM` 24 × 2048 × 32. Context information, seeded draws, band 64–246: `galactica-1.3b` reads **+4.6439** on text and **+0.0481** on protein, so its protein mode is **unmeasurable on this cohort** rather than failing; InstructProtein reads **+2.0490** on protein and **−15.9991** on text, placing a mean **96.3%** of its probability mass on its own twenty residue tokens at every scored text position — a collapsed text mode, not merely a poor one. The threshold-free control is reversal: the checkpoint that reads sequence directionally pays **2.1199** nats to have it reversed, and the two that do not pay **0.0564** and **0.0901**. Architecture is excluded by construction; what differs is training corpus and adaptation stage. **A bridge now exists (EXP-R2-152, 2026-08-10).** On the ProLLaMA three-point lineage — one architecture, one tokenizer, 32 × 4096 × 32 — `ProLLaMA_Stage_1` and `ProLLaMA` are **measurable in both modes**. The lineage also localises the cost: the protein continued-pretraining stage spends **4.69 nats/token of text, 85% of the base model's text context information**, to buy a protein mode worth +0.55, while the instruction stage costs a further 0.10. **Corrected the same day from the arm's own training scripts:** stage 1 is not parameter-limited where it matters — it carries `modules_to_save="embed_tokens,lm_head"`, fully retraining the embedding and output head at 45% of its trainable budget, while stage 2 is pure LoRA. **The stage that costs 4.69 nats is exactly the stage that retrains the vocabulary interface, and the stage that costs 0.10 does not touch it**, which points at the input/output interface rather than the transformer body and is consistent with L8. The base model's reversal cost is **−0.0013** and the adapted stage's **+0.1442**, so directional protein structure is absent before the adaptation and present after it. Protein magnitudes are per token at 1.533 residues/token and are **not comparable** with per-residue arms; the lineage's text comparison is exact. Qualification admits an arm to measurement and asserts nothing about what it knows. |
...
| **6** | **Fixed-depth width ladder on the protein side.** | whether capacity is excluded a second time on a clean design | The existing protein ladder confounds depth with width, so no protein result can currently separate them. Requires admitting a new architecture family and is ranked below the training-free work for that reason. |
| **7** | **Retrieval decomposition at fixed weights.** Sweep a retrieval mixing weight against a homology ceiling on the one released model whose retrieval prior is provably a per-position profile. | restates F10 as a decomposition | Cheap and it sharpens a citable claim, but F10 already stands; this improves its form, not its status. |
...
### D3 — Toward adapted methods

**Not earned.** No proposal is advanced that is not traceable to a catalogued limitation, and the two standing rejections of §7 are unchanged.
...
| **D3.g** | **Language-mediated causal concept alignment, candidate only.** Freeze both decoders and use genuine protein-sequence–curated-description pairs to map sequence-level or declared residue-level protein activations into a pre-declared text concept subspace or a shared concept bottleneck. Raw activation equality is not an estimand: each side is whitened or calibrated to concept percentiles or attainable causal-response units before comparison. Run mean, affine or orthogonal, nearest-neighbour retrieval and shuffled-pair baselines first; train a small bottleneck Adapter MLP only if non-linearity adds family-disjoint value, and report its capacity beside every result. Pass requires the adapter to beat every baseline and a rank-matched control, survive description and homology leakage checks, preserve unrelated concepts, transfer at least one graded concept intervention in the predicted direction, and reproduce on an unseen protein family. The language model may verbalise the aligned concept and predict an intervention outcome; protein-model intervention and external biological evidence, not linguistic plausibility, decide correctness | 5–12 GPU-h after a paired-data power audit; zero if the affine or retrieval baseline saturates the target |
...
- **Direct text-versus-protein feature crosscoders.** A model-diffing crosscoder compares two models on the same inputs; text and protein decoders share no such input, so that object remains undefined. D3.g is not a revival: it uses genuine sequence–description records to align pre-declared concepts after sequence-level aggregation, not token positions or supposedly identical hidden features. A direct crosscoder remains legitimate only where input pairing actually exists — one model before and after fine-tuning, or retrieval toggled on and off.
- **Attribution graphs without ground truth.** An attribution graph over protein
  features with no DMS, structural or functional referent cannot be validated, and
  L7 already prices the frozen-attention approximation it rests on.
