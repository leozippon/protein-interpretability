...
>
> *ProtGPT2's 4.5× is EXP-R2-126's K=4 figure and it reproduces exactly — both ρ endpoints to the digit — once its four draws are found: two in the single-arm `d2c_protgpt2_dfix_*` roots and two in the panel root, all at one condition. Recomputed through the module over **every** qualifying artefact the arm has, K=8 gives **6.3×**. The two differ because **ProtGPT2's retrieval is bimodal across draws** — four at 0–2, four at 5–7, nothing between — so its median moves with K instead of converging, which is a sharper statement than "the widest spread on the panel" and means **no point value should be quoted for this arm**. Its side of the classification is unaffected at K=4, 6 and 8. gpt2-large on the same grid and condition spans 6 to 8 over nine draws.*
>
