...
| **D3 — methods for biological knowledge and discovery** | Can an interpretable and causally faithful measurement distinguish learned biological knowledge from retrieval or surface correlation, then produce a testable biological hypothesis? | D3.b, D3.g, D3.h, and D3.i remain gated candidates; D3.e and D3.f remain closed. A proposal must answer a declared biological knowledge question or a reproducible measurement failure. A joint checkpoint provides a controlled bridge, but shared weights do not define a cross-modal target or relax the requirements for real sequence–description pairs, calibrated concepts, causal intervention, retrieval controls, and external biological evidence. |
...
|---|---|---|---|
| **1** | **Neuron-basis baseline.** Circuit tracing on raw MLP **pre-down-projection** activations, no dictionary trained, on the architecture-matched non-gated triple gpt2-large / ProtGPT2 / ZymCTRL. | F11's proximal cause | The only single experiment that **discriminates** the two live hypotheses: if neurons work on protein, the failure is the learned dictionary and is method-attributable; if neurons fail as badly, protein MLP computation is genuinely dense and no sparse basis will recover it, which is a model-property finding. Training-free. Two independent readers converged on it. **Must use the pre-down-projection tensor** — the post-down-projection one is far less sparse and would manufacture a false "proteins need dictionaries" conclusion. |
| **2** | **Matched-perturbation null.** Perturb each MLP output by a random vector matched to the transcoder's own error in **both norm and angle**; report ΔCE and KL beside the dictionary's. | whether the error is large or *pathologically directed* | EXP-R2-156 established that protein absolute damage is 8–17x the text arms' per symbol. This says whether that damage is what any perturbation of that size would do, or whether the protein dictionary's error is aimed at the load-bearing subspace. One extra forward pass per arm, no training. |
| **3** | **Collision null for the induction census.** Score the same heads on length- and composition-matched probes carrying no planted repeat; read each arm against its own null rather than against uniform attention. | closes the risk EXP-R2-155 opened | The panel-level half of the induction claim may not be quoted as a modality contrast until this is run. Forward passes only. |
...
| **6** | **Same-input Model Diffing Crosscoder (D3.h).** Compare matched ProLLaMA stages separately within each mode, only after exact input, position, layer, and activation-site correspondence is established. | which stable features protein adaptation introduces, removes, or preserves | The base protein mode is only an adaptation reference; it cannot carry a knowledge claim. Simpler baselines precede Crosscoder training, and reconstruction alone stops at a representation result. Full admission and stopping rule: §8. |
...
**On part 3 (methods for biological knowledge and discovery).** No proposed method has yet passed causal, retrieval-aware, and independent biological validation, and no new biological knowledge has been established. D3.b, D3.g, and D3.h are gated candidates tied to explicit knowledge questions and measured interface or translation problems; D3.e and D3.f remain closed. A candidate is a testable direction, not a result.
...
| Method-intrinsic non-transferability | **strongest** — every instance demonstrated on the text control, and it has now absorbed three estimands that were first read as modality differences: the induction census's depth confound (F5), the copy-suppression selector against its own layer index (L22), and the replacement causal-rank gate (L27) |
...
The deliverable. **Twenty-seven** limitations are catalogued in §5 as L1–L27, each scoped as **method** or **transfer** under that section's organising rule — a limitation demonstrated on the text control is a property of the method, one appearing only on protein arms is a property of the transfer. No row carries any other scope. *(This preamble read "Twenty-three limitations … each scoped as method, model, data or interface" until 2026-08-09; the count was stale and the wider vocabulary was never what the `scope` column uses. The rows are unchanged and are not renumbered — L22 to L26 appear out of order in the table, which is how they were added.)* What remains:
...
| **D3.b** | **Concept-aligned Lens / semantic aperture, candidate only.** Motivated by the confirmed low-rank output aperture of residue-tokenised models: replace raw-token Logit Lens or DLA comparisons with projections onto a pre-declared property basis. Within protein models this basis may include hydrophobicity, charge, side-chain volume and externally measured structural labels; a text–protein comparison is admissible only on modality-neutral abstractions such as copying, distance dependence or constraint satisfaction, because biological properties have no text analogue. A candidate result must beat shuffled-property and rank-matched controls, survive a causal intervention rather than only decode, and reproduce family-disjoint. Until those gates pass it is a proposed interface correction, not an achieved method or a cross-model coefficient | 3–8 GPU-h for a bounded pilot |
