...
| **D2.c** | **Answered (EXP-R2-126).** Eleven arms, one condition, both layout guards, A1 PASS throughout, K=3–5 per arm. **What survives the cross-lab control is one statement**: a prediction-addressed-attention census on a ProGen2 decoder does not recover the causally important heads above its *own* chance level (median 0.0–1.1× over 15 draws, two pretraining corpora), where every text decoder measured exceeds its own by ≥3.6×. **What does not survive as a resolved separation** is the modality ordering on either correlation: all-grid gap **+0.0804** and depth-controlled gap **+0.1416** at the most adverse draws, each smaller than one boundary arm's own spread across draws. Full statement, method and record in §D2.c below | spent; ~35 arm-draws across both machines |
...
**Three limits that do not move.** ZymCTRL is structurally excluded from any window ProtGPT2 can share (blocker 1 below), so the protein side is ProtGPT2 plus one lineage. ProtGPT2 is intermediate on every statistic and carries the panel's least stable retrieval (0, 0, 5, 6 over four draws) — the protein-side cohort sensitivity of §5.05(b) on a fifth statistic. And `hit@20` has a grid-dependent ceiling, so the panel holds exactly **one** within-grid cross-modality retrieval comparison — the matched pair at 720 heads, gpt2-large 6,7,7,8 against ProtGPT2 0,0,5,6, whose ranges overlap.
