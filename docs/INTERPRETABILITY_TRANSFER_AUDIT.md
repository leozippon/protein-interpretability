...
| **D2.c** | **Answered (EXP-R2-126), and the answer is about the tokenizer rather than a lineage (EXP-R2-128).** Eleven arms plus ZymCTRL, one condition, both layout guards, A1 PASS throughout, K=3–5 per arm and K=4 on ZymCTRL. **What survives the cross-lab control is one statement**: a prediction-addressed-attention census on a **residue-tokenised** protein decoder does not recover the causally important heads above its *own* chance level — median 0.0–1.1× on three ProGen2 arms and 0.9× on ZymCTRL, across two architectures, three scales, three corpora and both conditioning states — where every text decoder measured exceeds its own by ≥3.6× and the one subword-tokenised protein decoder reads 4.5×. Architecture and conditioning are ruled out; **residue tokenisation remains collinear with protein modality**, which only a byte-level text arm can break (EXP-R2-129). **What does not survive as a resolved separation** is the modality ordering on either correlation: all-grid gap **+0.0804** and depth-controlled gap **+0.1416** at the most adverse draws, each smaller than one boundary arm's own spread across draws. Full statement, method and record in §D2.c below | spent; ~39 arm-draws across both machines |
