...
| F8 | The twenty-three catalogued limitations of §5, each scoped as method, transfer, model, data or interface | as cited per row in §5 | **frozen** |
...
**The strongest directly measured part-2 result available is now EXP-R2-071/072/077/079: the prefix-matching induction census is a causal proxy on text decoders and is not one on protein decoders.**
...
>
> *ProtGPT2's 4.5× is EXP-R2-126's K=4 figure and it reproduces exactly — both ρ endpoints to the digit — once its four draws are found: two in the single-arm `d2c_protgpt2_dfix_*` roots and two in the panel root, all at one condition. **At K=13 the arm reads 7.2× its own chance**, on `0,1,2,2,3,3,4,5,6,6,7,7,8`, with 12 of 13 draws at or above chance. A bimodality recorded here at K=8 — four draws at 0–2, four at 5–7 — **was a hole in a small sample and is withdrawn**; the six draws run to test it closed the gap. What survives is that **this arm's retrieval is broadly dispersed across corpus draws**, spanning 8 hits where gpt2-large spans 2 on the same 720-head grid at the same condition. That is the protein-side cohort sensitivity of §5.05(b) on a seventh statistic and on the designed matched pair, and it is held as an observation rather than a measurement until the text control reaches comparable K — an arm with more draws has more opportunity to show its tails.*
>
...
**Reordered 2026-08-06.** The foundational programme has reached the point where
another draw of an instrument this document has already characterised cannot
change a conclusion, and the panel work is closed accordingly (D2.c, D2.d). What
the programme has *not* done is measure itself against the current external state
of the art, and that is now the main line: **D2.g reproduces ProGenMech under this
repository's own gates**, because a reproduction that only re-derives an author's
headline number adds nothing this catalogue does not already have, while one that
applies attainability, denominator validity and power discipline to it can say
something the original cannot. The remaining D3 items keep their gates and move
behind it.
