...
Use the same ESMFold2 instrument as EXP-R2-232: staged `biohub/ESMFold2-hf`, single-sequence, no MSA, isolated folding runtime, bfloat16, seed `20260905`, the staged checkpoint's own loops and diffusion settings, highest-pTM sample, and CA pLDDT converted from the library 0–1 scale to 0–100. The natural/shuffled pilot and separately held-out natural controls must be re-attained on ESMFold2; v1 calibration does not transfer. This is a shared control panel, not ProGen3-specific class-oracle validation. Report unmatched length/sequence regimes and do not borrow a claim of experimentally measured folder/nonfolder specificity.
