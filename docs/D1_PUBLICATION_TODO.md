...
- [ ] Commit and push focused code/documentation changes; preserve unrelated user edits to `AGENTS.md` and `CLAUDE.md`, and preserve the user's changes in `summary.md`.
- [ ] Fulfil the explicit open-source request: the target GitHub repository is currently **private**. The bounded current-file/history publication check and original-code MIT licensing are complete; preserve third-party terms and keep the manuscript private, then make the requested code repository public after final changes pass review.
