...
**Status:** methods only; asserts no results. **Scope:** estimands and metrics of entry points 01–14 only — the twelve registered foundational stages plus the two offline induction analyses, 12 and 13. It does not specify later external, campaign, or joint-model stages; the complete executable inventory and forward scope are in `docs/RESEARCH_PLAN.md` and `scripts/transfer/panel_contract.py`. Findings and their status are in `docs/INTERPRETABILITY_TRANSFER_AUDIT.md`, which wins over both.
