...
| `TRANSFER_KMER_BACKGROUND_DIR` | Pinned UniRef50 k-mer background for stage 37 |
| `TRANSFER_HIGH_ORDER_BACKGROUND_DIR` | k = 1..7 UniRef50 background for stage 37 |
...
Everything above assumes dispatch is cheap: one invocation per cell, each polled and pulled from the workstation. When the transit link is unstable that assumption multiplies the exposure, because an N-round campaign then needs N successful dispatches and N successful polls. Two committed scripts remove most of those crossings. **Each script's own header is the itemized record of what has been verified and what still needs checking; this paragraph does not assign a joint verification status.**

`h200_campaign_queue.sh` runs a whole campaign inside the pod. It is dispatched once — detached with `setsid nohup`, the same detachment the external-baseline driver gives a stage and the one that survived a six-hour outage — and then reads a tab-separated manifest of cells and runs them slot by slot: cells in a slot run concurrently, one per declared resource, and the next slot starts only when every cell of the previous one has exited. The `gpu` column is a card index or the literal token `cpu`. Integer rows keep the 2026-08-17 in-pod GPU path: `cuda:N` injection, nvidia-smi visibility, and the idle-card refusal. A `cpu` row is an explicit CPU resource, not a fictitious card and not a fallback from a busy GPU; the runner injects `--device cpu` and, after `h200_env.sh` and the cell env, exports `CUDA_VISIBLE_DEVICES` empty. CPU-only manifests do not require nvidia-smi; mixed manifests still apply the GPU gates to GPU rows. Local real-shell fixtures cover the CPU path; it has not been run in an H200 pod. CPU rows have no idle-card bumper, so an interrupted cpu cell is not safe to re-dispatch merely because the lock PID is gone. The rest of the per-cell invocation is the driver's: the same interpreter, the same `h200_env.sh` sourcing from the cell's own snapshot, and the same `results/external_baseline/<run-id>/<label>` and `logs/external_baseline/<run-id>_<label>.log` paths. Where a campaign ships one manifest per admissible card — the D2.i manifests differ in nothing but the `gpu` column — those files are **alternatives and not parallel lanes**: dispatch exactly one, because they share a campaign name and therefore the per-campaign lock, and share their labels and therefore their output directories, so dispatching more than one is either refused or duplicates the same cells. A campaign whose cells are pinned to different commits passes `--snapshot <key>=<dir>` more than once; the runner's header carries the worked case, where EXP-R2-207's trainer cells are pinned to a commit that predates `32_crosscoder.py` and so cannot share a freeze with the Crosscoder cells.
...
`watch_campaign_queue.sh` reads that status file without touching the payload. It polls the detached campaign through the access layer every `--interval` seconds (default 600), echoes the metadata block, and exits 0 with `CAMPAIGN_TERMINAL <campaign>` once the tally reports `pending=0` and `running=0`; until then it keeps polling, and a status file whose tally line is absent reads as still running rather than as a failure. `--campaign` names the campaign whose default status path (`<campaign>.status.tsv` beside its logs) is read — the script's own default is the extra-model context-information campaign it was written for — and `--status` names a status file explicitly. `H200_POD` is required in the current shell and `H200` overrides the access-layer CLI; no pod name is written anywhere. It is a read-only observation loop: it pulls no artefact and stops no cell, so `CAMPAIGN_TERMINAL` says only that the queue stopped moving and the records still have to be collected and admitted as below.
