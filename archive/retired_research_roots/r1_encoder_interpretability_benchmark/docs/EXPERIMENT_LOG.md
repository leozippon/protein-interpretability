...
- Reference: `r1_encoder_interpretability_benchmark/scripts/11_full_vector_classifier.py`
...
- Reference: `r1_encoder_interpretability_benchmark/scripts/12_annotated_feature_classifier.py`
...
- Reference: `r1_encoder_interpretability_benchmark/scripts/13_position_spread_analysis.py`
...
- Reference: `r1_encoder_interpretability_benchmark/scripts/14_ensemble_annotated_llr.py`
...
**Method:**
- Updated `r1_encoder_interpretability_benchmark/scripts/16_mechanism_classifier.py` to report both
...
**Method:**
- Updated `r1_encoder_interpretability_benchmark/scripts/21_proteingym_sae_followup.py` to report both the
...
**Method:**
- Added `r1_encoder_interpretability_benchmark/scripts/24_mechanism_feature_audit.py`.
...
- Remote log:
  `/oss-pvc/zhk_zip/biocc/benchmark_encoder/logs/runtime/t1d_annotation_firing_20260503.log`
- Fixed expansion log:
  `/oss-pvc/zhk_zip/biocc/benchmark_encoder/logs/runtime/t1d_expand_annotation_fixed_20260503.log`
...
- Remote log:
  `/oss-pvc/zhk_zip/biocc/benchmark_encoder/logs/runtime/t1c_indel_mechanism_20260504.log`
- Remote output:
  `/oss-pvc/zhk_zip/biocc/benchmark_encoder/results/variant_effect/indel_mechanism_predictions_20260504.jsonl`
...
## EXP-R1-EXT-005: Gene-Level Mechanism Gate (2026-05-12 CST)

**Question:** Does aggregating SAE perturbation features to the gene level
support a LOF/GOF/DN mechanism claim under family-level holdout?

**Method:** Added `scripts/39_gene_level_mechanism.py`. The run aggregates
variant-level SAE perturbation signatures per gene using mean, top-quartile,
and max summaries, then evaluates multinomial logistic regression under a
dominant-Pfam-family holdout proxy. The repository has Pfam family intervals
but no Pfam clan map, so this is not a full clan-level holdout.

**Result:** 253 genes evaluated: LOF 142, GOF 67, DN 44. Macro-AUC 0.5665,
macro-F1 0.3769, accuracy 0.4743. Per-class AUC: DN 0.5279, GOF 0.5879,
LOF 0.5837.

**Interpretation:** The acceptance gate fails. The R1 gene-level mechanism
headline should be dropped or relegated to negative diagnostics.

**Output paths:**
- `results/variant_effect/gene_level_mechanism_20260512.json`
- `results/variant_effect/gene_level_mechanism_20260512.md`

## EXP-R1-EXT-006: Indel Competitor Staging Attempt (2026-05-12 CST)

**Question:** Can the current IndelMissense v1 records be matched to dbNSFP
CADD/REVEL scores for a head-to-head indel competitor table?

**Method:** Added `scripts/40_indel_competitor_attempt.py`. The script checks
the IndelMissense v1 record schema and the staged dbNSFP GRCh38 BGZF header,
then scans the first 10,000 dbNSFP rows for indel-like ref/alt lengths.

**Result:** dbNSFP is staged and exposes CADD/REVEL columns, but the current
6,649 IndelMissense v1 records are protein-HGVS-only and lack chrom/pos/ref/alt.
The dbNSFP first-10k scan found 0 indel-like rows.

**Interpretation:** A defensible CADD/REVEL head-to-head cannot be run for the
current pass. Rebuilding from ClinVar VCF/genomic coordinates is required if
this comparison is reopened.

**Output paths:**
- `results/variant_effect/indel_competitor_attempt_20260512.json`
- `results/variant_effect/indel_competitor_attempt_20260512.md`

## EXP-R1-OPS-007: Canonical directory and document migration (2026-07-16)

**Scope:** Structural migration only; no scientific result was recomputed or
reinterpreted.

**Changes:** Renamed the live root to
`r1_encoder_interpretability_benchmark/`; moved the current plan, technical
notes, and legacy notes under `docs/`; moved the retained InterPLM paper under
`literature/background/`; added project/document indexes; and renamed the H200
entry points to `run_h200_batched.sh` and `submit_h200_sae.sh` to remove
duplicate numeric prefixes.

**Verification:** Active path references, Python syntax, shell syntax, shared
benchmark preflights/table generation, nested Git status, and live symlinks are
checked from the repository root. Historical result metadata and remote
`Research1` storage names remain provenance.
