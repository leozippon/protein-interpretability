...
Status: blocked by missing competitor assets; available local baselines were
recomputed with bootstrap CIs.
...
T1-A is not accepted because reviewer-facing competitor files are not staged.
The local SAE+LLR pathogenicity result is strong, but the full head-to-head
claim must wait until the four external baselines are staged and calibrated.
...
Status: completed for all currently accessible resources; gated PrimateAI-3D remains pending.
