...
- PrimateAI-3D is treated as unavailable because gated access could not be
  obtained for this analysis round.
- T1-A is finalized over accessible baselines only: SAE-LR, ESM-2 LLR,
  SAE+LLR, AlphaMissense, gMVP, and ESM-1v.
- Added
  `Research1/results/variant_effect/t1a_final_available_baselines_no_primateai_20260510.{json,md}`
  as the final bookkeeping artifact for the no-PrimateAI analysis pass.
- Created `analysis_exports/20260510_no_primateai/` to collect necessary
  result summaries, tables, logs, and project docs for local analysis. Large
  checkpoints, model weights, and large binary perturbation/signature pickles
