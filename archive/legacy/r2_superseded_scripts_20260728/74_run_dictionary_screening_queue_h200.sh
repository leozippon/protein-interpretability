...
PROJECT="${ROOT}/code_p0_2_dictionary_controls_bf16_r2/r2_decoder_sparse_readout_audit"
CODE_ARCHIVE="${ROOT}/code_archives/r2_p0_2_dictionary_controls_bf16_payload_r2.tar"
...
OUTPUT_ROOT="${ROOT}/p0_2_dictionary_controls_bf16_r2/screening"
...
RUNNER_SHA256="fec4ed0c2cdf0881fe922e7f46c2cc13393d82dad22da8dde50ebcaf5ac99204"
...
verify_deployment() {
  ...
  archived_manifest_sha="$({
    tar -xOf "${CODE_ARCHIVE}" \
      r2_decoder_sparse_readout_audit/CODE_CONTENT_SHA256SUMS
...
run_one() {
  ...
  python3 - "${output}/results.json" "${model}" "${method}" \
    "${PROFILE_SHA256}" <<'PY'
import json
import sys
from pathlib import Path
...
model, method, profile_sha256 = sys.argv[2:5]
...
expected_candidates = {"relu_l1_sae": 5, "gated_sae": 10}[method]
post_candidate_allocations = []
if (
    not isinstance(candidate_validation, list)
    or len(candidate_validation) != expected_candidates
):
    raise SystemExit("screening terminal result has an incomplete candidate grid")
for candidate in candidate_validation:
    training = candidate.get("training") if isinstance(candidate, dict) else None
    if (
        not isinstance(training, dict)
        or type(training.get("post_candidate_accelerator_memory_allocated_bytes"))
        is not int
        or training["post_candidate_accelerator_memory_allocated_bytes"]
        > 128 * 1024**2
    ):
        raise SystemExit("screening candidate did not release accelerator memory")
    post_candidate_allocations.append(
        training["post_candidate_accelerator_memory_allocated_bytes"]
    )
if len(set(post_candidate_allocations)) != 1:
    raise SystemExit("post-candidate accelerator-memory baseline changed")
PY
