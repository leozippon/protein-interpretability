...
本表按谱系列出实际使用或本次指定的 checkpoint。同一谱系的笼统覆盖不等于每个尺寸都已跑过同一实验；列入本表也不等于接口、数值或实验已经获得资格，也不等于已有完成结果。

上下文长度是各 checkpoint 自己声明的服务上限，不是本项目的计分窗口：上下文信息资格测量默认在 384 token 窗口内计分。该值取自各 checkpoint 的 config，tokenizer 与 config 冲突时以 config 为准。
