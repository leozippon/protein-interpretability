...
EXP-R2-225 扩展的 7B 接口资格仍未作为该扩展运行；派生预算资格代码不是 7B 运行。ProteinGym 上 ProteinGLM、Galactica FP32 v2 与 RITA-xl 的数字见上表。RITA-xl 配对已从冻结清单副本重跑。ProtGPT3-1.3B 的原生格式资格门正在排队，ProteinGym 在其后打，门过不过都打。ProGen3 的 MegaScale 行仍缺稳定性评分、逐 checkpoint 的语料识别记录和评分分层标注。这些不替代方向二、三的下一步。
