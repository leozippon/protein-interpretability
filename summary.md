...
能力比较先要有一个统一的可测性入口，否则不同模型的数字根本不可比。入口建立之后，本节依次问：联合模型是否真的同时支持两种模式，蛋白模型在外部任务上能做到什么，更大的 checkpoint 或更多的训练阶段是否改变结论，以及模型除了给序列打分之外能否按要求生成。
...
| 实验 | 模型 | 数据(格式、示例、数量) | 怎么做 | 结果 | 状态与局限 |
| --- | --- | --- | --- | --- | --- |
| Galactica / InstructProtein 双模式能力 | Galactica 125M/1.3B/6.7B/30B、InstructProtein；Galactica-1.3B 与 InstructProtein 都是 24 层、宽 2048 的 OPT 形状 | 文本为 OpenWebText/WikiText-103 记录；蛋白为 Swiss-Prot 64–246 aa，如 `MKT...`。每个 checkpoint/mode 评分 128 条，另用 400 条独立记录拟合 unigram；后加入的两个 rung 复用同一抽样种子、同一队列摘要和同一参考集大小 | ①确定两种模式的原生包装。②分别测相对 unigram 的上下文信息。③反转蛋白序列并检查方向代价。④检查文本输出概率是否仍落在文本 token。⑤从逐记录统计做 2,000 次近重复组 bootstrap，并扣除参考集重采样造成的区间位移。⑥对更大的两个 rung 先过固定序列接口门，再用同一识别规则逐 rung 判定 | Galactica-1.3B 的蛋白模式信号很小：+0.0477 nats/token，位移校正后的区间下界为 +0.0387；6.7B 和 30B 在同一队列上也在两种模式下通过识别，蛋白模式区间依次升到 +0.1576–+0.2457 和 +0.3425–+0.5417；125M 的蛋白模式上下文信息为负，不可识别，三个较大 rung 的反向蛋白条件也都不可识别。InstructProtein 的蛋白模式可测，但文本位置约 96.3% 概率质量落在残基 token | 已完成。Galactica 现在是覆盖四个规模的第二条合格双模式谱系；但低幅度信号不自动形成可靠的双模式机制桥，InstructProtein 文本模式仍关闭。三个 rung 之间没有形成任何配对量，蛋白模式信号的上升是三次独立识别的并列，不是门，也不能归因于参数量 |
...
### 追踪 checkpoint 与训练阶段带来的变化

| 实验 | 模型 | 数据(格式、示例、数量) | 怎么做 | 结果 | 状态与局限 |
| --- | --- | --- | --- | --- | --- |
| ProGen2 阶梯上的描述性能力门 | ProGen2 medium→large→xlarge，约 7.65 亿、27.8 亿、64.4 亿参数 | 复用已冻结的 ProteinGym 与 MegaScale 队列。受 1024 位置窗口限制，DMS 可分析集为 201 assays / 163 families；MegaScale 为 130 个无同源设计 / 40 个 series 与 266 个天然域 / 124 clusters；组 bootstrap 2,000 次、seed 20260825 | ①先在 bfloat16 和 float32 两个精度上通过严格加载与固定序列 NLL 的接口门。②按原生方向标记渲染，不同评分方向分层。③只对同谱系相邻 rung 判定预注册的复合门。④DMS 要求较大 rung 的 MODEL−LOOKUP 下界大于零，且 raw Spearman 配对增量下界大于零；MegaScale 要求设计侧和天然侧同时超过 hydropathy 与 BLOSUM62 | 四个门全部为 false。三个 rung 的 MODEL−LOOKUP 是 −0.0043、+0.0122、+0.0039，区间都覆盖零，没有一个 rung 越过同一条 profile 通道；raw Spearman 是 +0.3557、+0.3722、+0.3639，最大的 rung 不是最好的。在 130 个无同源设计上，三个 rung 都显著低于 hydropathy 和 BLOSUM62 两个序列基线 | 已完成。设计侧的结果在第三、第四个 rung 上重复了 F12 而不是推翻它。false 与 true 一样都不是关于参数量的结论：深度、宽度、参数量和训练混合同时变化。LOOKUP 因为没有完全搜索 UniRef90 与 BFD30 而偏向模型，偏向之后仍然不显著才是更强的说法。两个下界不含零的配对增量只是辅助区间，不是门翻转。见 EXP-R2-224 |
| 更大公开 checkpoint 的第二阶段 | Wave A 的三条阶梯：Galactica 1.3→6.7→30B（联合）、Qwen2.5-base 0.5→7→32B（纯文本）、ProGen3 112M→3B（纯蛋白）。ProteinGLM-7B-CLM 与 RITA_xl 作为新谱系存在性点，属于尚未运行的 Wave B | 上下文信息复用各谱系已冻结的区块、种子和参考集；ProGen3 复用 ProteinGym 的 217 assays / 174 families；组 bootstrap 2,000 次、seed 20260825 | ①在出现任何分数之前，先用一次 pre-data 修订写定每条 wave 真正能喂到哪个端点。②每个 checkpoint 先过自己的固定序列接口门：严格加载、重复到 1e-6，乱序探针至少 0.25 nats 代价。③两条上下文信息阶梯按同一位移校正规则逐 rung 识别。④ProGen3 没有面板声明，改用逐 checkpoint 的加载自检带作为资格，并把两个 rung 都用当前打分阶段重新打分。⑤只有 ProteinGym 这一个端点带预注册的复合门 | Galactica 和 Qwen 两条阶梯都通过识别，但按构造不产生任何门判定，因为两条都不带适应度端点。ProGen3 阶梯返回了本项目第一个 `descriptive_gate_transition: true`：MODEL−LOOKUP 从 112M 的 −0.0801 [−0.1135, −0.0460] 变到 3B 的 +0.0555 [+0.0345, +0.0774]，raw Spearman 从 +0.2750 升到 +0.4106。112M 的重新打分复现了它此前的读数 | Wave A 已完成，Wave B 未运行，三条 wave 都不交付 MegaScale 行。ProGen3 的 LOOKUP 是外部 UniRef50 profile 基线而不是检索边界——该谱系声明的语料更大，所以这个对比偏向模型，越过它不等于学到了生物学。转变只描述两个具名 checkpoint，不是参数量的因果结论。ProGen3 与 ProGen2 使用不同的可分析集（217/174 对 201/163）和不同的打分口径（双向均值对单向求和），两条阶梯不合并、不放进同一列。见 EXP-R2-225 |
...
| 蛋白继续预训练买到了什么 | Llama-2-7B → ProLLaMA Stage 1 → Stage 2；三者同为 32 层、宽 4096，且 `tokenizer.model` 的 SHA-256 完全相同 | 复用已冻结的 ProteinGym 队列（seed 20260807、每 assay 最多 1,000 个变体、按 `mutant_digest` 固定）与 MegaScale 队列。三者共享 4096 位置窗口，最长野生型渲染后为 2,231 token，因此可分析集与声明队列相同，均为 217 assays / 174 families——与 ProGen2 阶梯的 201 / 163 不是同一个集合，二者不合并 | ①三个 checkpoint 一律用**不带指令**的 `Seq=<...>` 渲染：填入真实 superfamily 会把答案写进提示，编造一个更糟。②Stage 2 需通过严格加载、重复到 1e-6 的固定序列 NLL 自检，以及同一渲染下大于 0.05 nats/token 的反向代价。③base 作为**未适配下限**而非合格蛋白臂进入。④以 family 为单位做 2,000 次组 bootstrap（seed 20260826），判定与 ProGen2 阶梯相同的两个复合门 | 两条队列上的判定都是 False。ProteinGym 的 raw Spearman 从 base 的 −0.0071 走到 Stage 1 的 +0.1358，再到 Stage 2 的 +0.1507；base→Stage 1 的配对增量为 +0.1429 [+0.1150, +0.1699]，指令阶段只再加 +0.0149。但没有任何 rung 越过带语料的通道：LOOKUP 读 +0.3537，连不需要语料的 BLOSUM62 也有 +0.2097，三个 rung 的两个差值区间全部整体低于零 | 已完成。适配确实把排序移动了一大截，但排序移动不等于获得能力——打分阶段对三个 rung 的自有判定都是 `retrieval_dominated`。MegaScale 一侧完全不可读：天然对照在三个 rung 上都没通过，判定是 `uninterpretable_instrument_bound`，设计侧的数字按预注册照印但不得解释。不是知识证据、不是机制证据，也不能归因到适配中的任一单独成分——该阶段同时重训了 embedding 与输出头，占可训练参数的 45.0%。ProLLaMA 声明的 UniRef50 包含这些蛋白，本实验不排除检索。见 EXP-R2-226 |

### 检验条件生成与上下文使用

| 实验 | 模型 | 数据(格式、示例、数量) | 怎么做 | 结果 | 状态与局限 |
| --- | --- | --- | --- | --- | --- |
| 原生条件生成的方向性 | ZymCTRL（EC 标签）、ProLLaMA Stage 2（superfamily 指令）；无条件下限为 ProGen2-medium 与 ProtGPT2；文本正对照为 Qwen2.5-0.5B 与 Llama-3.2-3B | 类别队列按种子置换抽取：EC 取自该臂自己的标注语料（实测 244,343 条、5,594 个完整 EC 号，其中 349 个有 ≥200 条），superfamily 取自随仓库暂存的官方列表（1,885 个单一 superfamily 标签），各冻结 16 个类别；每个臂×类别×条件生成 200 条 | ①用统一冻结的采样配置，同一臂分别在**请求标签**、**错配标签**和无条件三种条件下生成。②用非神经的 HMMER/Pfam-A 在该发行版自己的 gathering 阈值上作主判定器。③每个类别先用真实蛋白与长度匹配随机序列校准判定器，未达标的类别在计分前退出队列。④文本对照用语言标签加 Unicode 字符集判定，判定器确定且无参数。⑤以**类别**为单位做 2,000 次 bootstrap，并在形成类别比率之前先做近重复合并 | 两个条件臂都返回“条件化把生成推向被请求的类别”。ZymCTRL 的请求减错配为 +0.8822 [0.7202, 0.9872]，14 个类别全部单独为正；ProLLaMA Stage 2 为 +0.1267 [0.0527, 0.2360]，15 个类别中 11 个为正。两个臂都不返回“标签只是移动了分布、并没有选中被请求的类别”，而后者正是这次测量存在的理由。文本正对照在确定性、零参数的判定器上达成：Llama-3.2-3B 的 12 个类别全部通过，Qwen2.5-0.5B 恰好 8 个，都不低于所需的 8 个 | 已完成，作为 F17 进入可引用集，范围限定为**由策展 profile 判定器在各臂自己的标注队列上做出的归属**，绝不是“生成了具有该功能的酶”。三条实测边界写进结论本身而不是放在旁边：ZymCTRL 生成物对 UniRef50 的最大 identity 中位数为 69.2%，所以高归属率与检索训练分布内的材料一致，本实验不区分二者；Pfam 只识别出 ZymCTRL 生成物的 0.960 和 ProLLaMA 的 0.135，所以 ProLLaMA 的 +0.13 处在约 0.135 的天花板之下，它的弱是判定器根本认不出来而不是认成了别的家族；近重复合并把 ZymCTRL 的 5,600 条样本压成 3,942 组，比率高的那一侧正是合并起作用的一侧。CLEAN 与结构协变量离线不可用，按不可用报告而不做替代，因此每个臂只有一个门。两个臂的比率从不相减。这是**原生条件接口**，不是 §9.1 已退役的内部特征 steering（那条线的结论是 0/8）。见 EXP-R2-227 |
| 上下文同源序列条件化 | gpt2-large 与 ProtGPT2（面板声明的匹配对）、ProGen2-small、ProGen2-medium；progen2-base（窗口 2048）与 ZymCTRL（渲染会给每条上下文强加 EC 标签）按名排除 | 蛋白目标取 Swiss-Prot 64–246 aa，同源序列按 DIAMOND identity 分档：≥90 直接排除，其余 70–90、50–70、30–50、<30；文本改用**按上下文占比匹配**的段落队列（80–130 个 BPE token）。每个臂保留 800 个单位，蛋白侧按 710 个近重复组、文本侧按 201 篇源文档重采样 | ①固定 1024 位置的 **token 预算**而非固定 k，k 作为结果报告。②四组同长度、同 k 的对照：组成匹配的无关天然序列、单残基打乱的同源序列、只占位置的固定填充、近重复筛除。③加**局部重叠筛查**（最长公共子串、共享 7-mer），因为全局 identity 无法屏蔽复制机制。④主要指标为无量纲的配对对比与家族归属 AUROC | 没有任何臂通过三条款复合门。ProtGPT2 和 ProGen2-small 确实能把真同源上下文与组成、长度匹配的无关上下文分开（合并 AUROC 0.786 和 0.653），但这个能力随局部重叠单调下降，到 30% identity 就落回机会：ProtGPT2 按四档读 0.915 / 0.935 / 0.770 / 0.525。预注册的决定性条款——效应必须在 `<30` 档的最低局部重叠层内仍然成立——在每个臂上都失败，而被排除的逐字重叠层是整个实验里最强的读数（0.955 和 0.925）。ProGen2-medium 反向移动，在 50–70% identity 上 AUROC 为 0.225 [0.172, 0.281] | 已完成，作为 F16 进入可引用集：冻结的单序列预训练解码器的上下文同源增益由局部重叠承载，去掉重叠就什么也不剩，而且承载它的复制机制是被测到的而不是只被筛掉的。这是关于四个具名 checkpoint 的行为结论，不是模态结论——ProtGPT2 曾在 FASTA 多序列窗口上预训练，这是该臂自己的混杂；ProGen2-medium 的反向是未解释的臂间差异，不构成任何规模门。所有已发表的正结果都来自**为该条件专门训练**的模型，所以阴性只约束冻结的解码器。仪器在决定层自己的 67 个单位上仍能分辨大效应，所以 `<30` 的零结果是关于这一层而不是关于分辨率的。见 EXP-R2-228 |
...
| 同一权重下两种模式的读出深度 | Galactica 1.3B 与 6.7B 的文本模式与蛋白模式；125M 因蛋白模式不可识别而不进入判定，30B 单独运行 | 与各 rung 自己的上下文信息资格队列同一批记录：128 条 Swiss-Prot 64–246 aa 与匹配的文本记录，20,866 个残基目标对 20,864 个文本目标，蛋白渲染精确到每个计分 token 一个残基 | ①用未调优的 logit lens 逐层读出，并要求 lens head 与模型自己末层分布的 KL 低于冻结的 1e-2 nats。②以 top-1 一致率跨过若干水平的相对深度作为主统计，另用跨度归一的 KL 深度作为第二个泛函。③以记录聚类 bootstrap 形成蛋白减文本的差。④用位置上限、朴素渲染和精度不变性三个控制给自己的混杂定价 | 两个可识别的 rung 都返回“蛋白模式解析得更晚”：相对深度上的蛋白减文本在 1.3B 为 +0.1381 [+0.1332, +0.1435]，在 6.7B 为 +0.1352 [+0.1292, +0.1413]。30B 被自己的仪器门挡住——它的 lens head 差到 8.569e-02 nats，超过冻结的 1e-2 容差，容差没有被放宽，阶梯按三个 rung 报告 | 已完成，不进入可引用集。这是同一套权重、同一个输出头、同一个 tokenizer 下的两种模式，是面板在别处做不到的对照；但没有拟合调优 lens，所以它分不开“计算解析得更晚”和“中间状态离最终基更远”，在调优 lens 复现出现之前不得按前者引用。预注册的第二个读数返回平的：蛋白模式自己的上下文信息在两个 rung 之间涨了四倍多（+0.0477 → +0.1994 nats/residue），深度差却几乎不动。朴素渲染控制在 6.7B 上吃掉约三分之二的差值。要拿回 30B 需要把整条阶梯改成 float32 重跑，那是另一次测量。见 EXP-R2-229 |
...
这一轮的读数放在一起有一条值得写下来的线索：能力差异确实存在，但不跟参数量走。ProGen2 到 64.4 亿参数仍然没有越过同源 profile 通道，ProGen3 在 30 亿参数上越过了；从 70 亿文本模型适配而来的 ProLLaMA 把蛋白排序移动了一大截，仍然远低于同一类通道；上下文里的增益是复制而不是同源；而同一套权重下两种模式的读出深度差，在蛋白侧上下文信息上涨四倍时几乎不动。有两条限制必须跟着这句话一起读：ProGen3 与 ProGen2 使用不同的可分析集和不同的打分口径，两条阶梯从不合并，上面那句不是“3B 打败了 6.44B”；而唯一随 rung 单调上升的量是纯文本的上下文信息阶梯，它按构造不构成门。就现有证据能说的是，与差异对应的是语料与训练目标、接口形式和模态，而不是规模本身。
...
| 不能声称 | 不能断言模型已经学到、没有学到或只是在记忆；不能把无整序列同源、超过随机基线、合理生成物或结构预测写成知识证据。条件接口按请求的类别生成也不例外：那是行为测量，既不证明生成物有功能，也没有把类别选择与检索训练分布内的材料分开 |
| 方向一、二下一步 | 方向一这一轮把所有已冻结的预注册都读了出来，没有留下未运行的项目。剩下的是第二阶段的 Wave B（两个新谱系的存在性点）和一个有条件的 ProGen3 稳定性行，两者都还缺前置条件。方向二新增的同权重双模式读出深度对照有一个明确的下一步：拟合调优 lens 复现，否则这个读数不能按“计算解析得更晚”引用；要拿回最大的那个 rung 还需要把整条阶梯改成 float32 重跑。同权重模式子空间实验已经完成但没有得到许可的共享或分离子空间结论；除此之外仍然优先补充第二个可靠的多残基蛋白模型、尽可能匹配的 BPE 文本对照，以及不依赖 token 对齐的序列级测量，而不是扩大未通过判定门的子空间扫描 |
