...
| R3.5 | 3. 开发适配方法 | Language-mediated Causal Concept Alignment | 候选方法使用真实的蛋白序列–人工文本注释配对，将两类模型中经过校准的同一概念激活或因果响应对齐；先检查仿射和检索基线，必要时只训练一个冻结两侧模型的小型 bottleneck Adapter MLP。 | **候选；尚未实现** | 必须击败均值、仿射、检索、打乱配对和秩匹配控制，并通过概念干预、无关概念保持、同源性防泄漏及独立蛋白家族验证；语言流畅性不构成正确性证据。 |
