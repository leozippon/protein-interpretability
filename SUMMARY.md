...
|---|---|---|
| **EXP-R2-149：模态还是 tokenization？** ZymCTRL 作为第三个 36×1280 arm 进入 replacement 门控。它与 gpt2-large、ProtGPT2 架构完全相同，但**逐残基分词**（词表 458）且带 EC 条件前缀。两个语料 seed 的 transcoder 训练（各 22,000 步 / 66,758,421 个残基 token / 120.7 token per latent，位于 gpt2-large 的 132 与 ProtGPT2 的 108 之间）正在 H200 上运行 | ProtGPT2 的失败是否属于「蛋白模态」，还是属于 multi-residue BPE 分词 | **判定规则已于启动前写定（EXP-R2-149）：** 落在 0.13–0.16 蛋白区间 → 失败与分词无关，模态归因增强，且**此结论是决定性的**，因为 EC 泄漏（实测 2.4487 nats）、76.6% 的 cohort 污染与 1.22 nats 的低 clean NLL 三项偏置全部指向让该 arm 通过；≥0.80 → 「蛋白模态」过粗，需先补无条件 ZymCTRL 对照才能解读，**本身不可读**；介于两者之间 → 记为中间结果。**该 arm 无法分离分词与 EC 条件两个差异**，只能把模态解释与「分词或条件」区分开。因果排序指标已由 EXP-R2-148 撤出模态用途，本 arm 的交付物只有行为恢复率。**首个已 ADMITTED 的结果（自由线性基线，n=128）为 −0.0096 [−0.0162, −0.0029]，clean 0.8364 → ablated 3.0282，分母 2.1919。**它落在文本区间（+0.26 / +0.29）与另外两个蛋白 arm（−0.72 / −1.61）之间、几乎为零，因此上文 EXP-R2-144「两组不重叠」的表述只对该实验的四个 arm 成立：加入 ZymCTRL 后蛋白侧跨越 −0.01 至 −1.78，并非紧密的一组。该限定在 transcoder 结果产出后与整条结论一并重写，此处先记录，不提前解读。 |
